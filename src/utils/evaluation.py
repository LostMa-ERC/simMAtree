...
def evaluate_inference(true_params, results_dir, param_names=None):
    ...
    try:
        ...
        pp_samples_xr = xr.DataArray(posterior_pred_samples, dims=["sample", "stat"])
    ...
    for i, (name, true_val) in enumerate(zip(param_names, true_params)):
        ...
        norm_squared_error = (
            squared_errors[i] / (true_val**2) if true_val != 0 else squared_errors[i]
        )
    ...
    nrmse = np.sqrt(
        np.mean(
            [
                m["norm_squared_error"]
                for m in param_metrics.values()
                if m["norm_squared_error"] != float("inf")
            ]
        )
    )
    ...
    rel_errors = [
        param_metrics[name]["rel_error_pct"]
        for name in param_names
        if param_metrics[name]["rel_error_pct"] != float("inf")
    ]
    rel_error_names = [
        name
        for name in param_names
    ...
    plot_posterior_predictive_stats(
        pp_samples_xr, obs_value=obs_values, output_dir=results_dir
    )
    ...
    plot_marginal_posterior(
        posterior_samples, output_dir=results_dir, true_value=true_params
    )
